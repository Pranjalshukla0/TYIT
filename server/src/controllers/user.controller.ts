...
export const logoutUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (userId) {
        await redis.del(userId); // Only delete if userId exists
      }
      res.status(200).json({
        success: true,
...
export const updateProfilePicture = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // If the user already has an avatar, delete the old one first
      ...
      res.status(200).json({
        ...
        avatar: user.avatar, // Only return avatar, not full user object
      });
    } catch (error: any) {
      return next(new ErrorHandler(error.message, 400));
    }
...
export const updateUserRole = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (isUserExist) {
        const id = String(isUserExist._id); // Ensure id is a string
